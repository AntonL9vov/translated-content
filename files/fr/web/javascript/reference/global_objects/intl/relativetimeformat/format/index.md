...
{{JSRef}}La méthode **`Intl.RelativeTimeFormat.prototype.format()`** permet de formater une valeur avec une unité selon des options de locale et de formatage stockées dans l'objet {{jsxref("RelativeTimeFormat")}}.{{InteractiveExample("JavaScript Demo: Intl.RelativeTimeFormat.prototype.format")}}
